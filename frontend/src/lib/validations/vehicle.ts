...
export const vehicleSchema = z.object({
  ...
  bodyType: z.string().max(80).optional().nullable().transform((v) => (v === '' || v === 'ninguno' ? null : v)),
  ...
  vtvExpiresAt: z.string().optional().nullable().transform((v) => (v ? new Date(v).toISOString() : null)),
  patentExpiresAt: z.string().optional().nullable().transform((v) => (v ? new Date(v).toISOString() : null)),
  ...
  sellerEmail: z.string().max(120).optional().nullable().refine((v) => !v || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v), 'Email inválido'),
  plan: z.string().max(50).optional().nullable().transform((v) => (v === '' || !v || v === 'ninguno' ? null : v)),
  planExpiresAt: z.string().optional().nullable().transform((v) => (v ? new Date(v).toISOString() : null)),
  addonDestacado24hUntil: z.string().optional().nullable().transform((v) => (v ? new Date(v).toISOString() : null)),
  addonPrioritario7dUntil: z.string().optional().nullable().transform((v) => (v ? new Date(v).toISOString() : null)),
}).refine(
  (data) => {
